...
public static class Native {
    public static T Invoke<T, T2>(IntPtr library, params object[] pars) {
        ...
        if (funcPtr == IntPtr.Zero) {
            Debug.LogWarning("Could not gain reference to method address for " + typeof(T2).Name);
            return default(T);
        }
    ...
    public static void Invoke<T>(IntPtr library, params object[] pars) {
        ...
        if (funcPtr == IntPtr.Zero) {
            Debug.LogWarning("Could not gain reference to method address for " + typeof(T).Name);
            return;
        }
